...
function updateLanguage(lang) {
    ...
    document.querySelectorAll("[data-i18n]").forEach(el => {
        ...
        if (translations[lang][key]) {
            el.innerHTML = translations[lang][key]; // 使用 innerHTML 以保留 <b> 標籤
        }
    });
}

function updatePlaceholders() {
    console.log("Updating placeholders for language:", currentLang); // 確認語言變數


    
    document.querySelector("#login_email").setAttribute("placeholder", translations[currentLang]["enterYourEmail"]);
    document.querySelector("#login_password").setAttribute("placeholder", translations[currentLang]["login_password"]);
    document.querySelector("#reset_email").setAttribute("placeholder", translations[currentLang]["enterYourEmail"]);
}
